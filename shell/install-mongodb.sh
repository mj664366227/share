#linux mongodb自动安装程序 
#运行例子：sh install-mongodb.sh 3.0.5 /usr/local
...
if [ ! $mongodb_version ] || [ ! $mongodb_install_path ] ; then
	...
	echo 'for example: sh install-mongodb.sh 3.0.5 /usr/local'
	exit
fi
...
yum -y install libtool sed gcc gcc-c++ make net-snmp curl net-snmp-devel net-snmp-utils libc6-dev python-devel rsync perl bc libxslt-dev lrzsz
