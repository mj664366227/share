...
if [ ! -d $erlang_install_path/erlang ]; then 
	...
	./configure --enable-sctp --enable-kernel-poll --enable-smp-support --enable-threads --enable-halfword-emulator --enable-hipe --enable-native-libs --enable-m64-build --prefix=$erlang_install_path/erlang && make && sudo make install
