#linux git
#运行例子：sh install-erlang.sh /usr/local
...
if [ ! $erlang_install_path ]; then
	echo 'error command!!! you must input erlang install path...'
	echo 'for example: sh install-erlang.sh /usr/local'
	exit
fi
