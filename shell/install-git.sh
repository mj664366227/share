...
if [ ! -f $base_path/$m4.tar.gz ]; then
	wget -O $base_path/$datadumper.tar.gz http://o77dg0j7k.bkt.clouddn.com/$datadumper.tar.gz || exit
fi
...
if [ ! -d $php_install_path/m4 ]; then
	m4='m4-1.4.17'
	if [ ! -f $base_path/$m4.tar.gz ]; then
		wget -O $base_path/$m4.tar.gz http://o77dg0j7k.bkt.clouddn.com/$m4.tar.gz || exit
	fi
