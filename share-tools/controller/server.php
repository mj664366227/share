...
class servercontroller {
	...
	public function ip() {
		...
		// tool.lu
		$toollu = curl::post('http://tool.lu/netcard/ajax.html', array (
				'ip' => $ip 
		));
		...
		// 站长工具
		$chinaz = curl::post('http://ip.chinaz.com', array (
				'ip' => $ip 
		));
		...
		// 淘宝
		if ($ip_address) {
			$taobao = curl::get('http://ip.taobao.com/service/getIpInfo.php', array (
					'ip' => $ip_address[0] 
			));
