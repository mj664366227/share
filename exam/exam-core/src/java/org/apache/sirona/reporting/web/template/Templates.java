...
public final class Templates {
	...
	private static String project = "/" + FileSystem.getProjectName().substring(FileSystem.getProjectName().indexOf("-") + 1);
	...
	public static void init(final String context, final String filterMapping) {
		...
		velocityConfiguration.setProperty(RESOURCE_LOADER_KEY, //
				Configuration.getProperty(Configuration.CONFIG_PROPERTY_PREFIX + "reporting.resource-loader", //
		...
		if ("/web".equals(project)) {
			project = "";
		}
		if ("/admin".equals(project)) {
			project = "/upgather";
		}
		logger.warn("sirona init ...");
	}
	...
	public static void htmlRender(final PrintWriter writer, final String template, final Map<String, ?> variables) {
		...
		context.put("mapping", project + mapping);
		...
		final Template velocityTemplate = onlyBodyRendering ? engine.getTemplate("/templates/" + template, "UTF-8") : engine.getTemplate("/templates/page.vm", "UTF-8");
